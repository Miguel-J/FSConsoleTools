...
class GeneratePhpModel extends ConsoleAbstract
{
    ...
    public function run(...$params): int
    {
        ...
        $this->showMessage('Generating Model class file' . \PHP_EOL . \PHP_EOL);
        ...
        if (isset($params[0])) {
            switch ($params[0]) {
                ...
                case '--gen':
                    ...
                    $this->setDstFolder(\FS_FOLDER . (isset($params[3]) ? $params[3] : 'Core/Model'));
    ...
    private function generateModel(): int
    {
        ...
        if (\is_bool($status)) {
            trigger_error("Can't save " . $this->getDstFolder() . $this->getModelName() . '.php"' . \PHP_EOL);
            return $status;
        }
