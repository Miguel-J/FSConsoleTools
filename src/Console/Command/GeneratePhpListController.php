...
class GeneratePhpListController extends ConsoleAbstract
{
    ...
    public function run(...$params): int
    {
        ...
        $this->showMessage('Generating List Controller class file' . \PHP_EOL . \PHP_EOL);
    ...
    private function generateModel(): int
    {
        ...
        $status = $this->saveFile($this->getDstFolder() . 'List' . $this->getModelName() . '.php', $txt);
        if (\is_bool($status)) {
            trigger_error("Can't save " . $this->getDstFolder() . 'List' . $this->getModelName() . '.php"' . \PHP_EOL);
            return $status;
        }
