...
class GenerateXmlEditModel extends ConsoleAbstract
{
    ...
    public function run(...$params): int
    {
        ...
        $this->showMessage('Generating XMLView Edit file' . \PHP_EOL . \PHP_EOL);
        ...
        if (isset($params[0])) {
            switch ($params[0]) {
                ...
                case '--gen':
                    ...
                    $this->setDstFolder(\FS_FOLDER . (isset($params[3]) ? $params[3] : 'Core/XMLView'));
    ...
    private function generateModel(): int
    {
        ...
        $status = $this->saveFile($this->getDstFolder() . 'Edit' . $this->getModelName() . '.xml', $txt);
        if (\is_bool($status)) {
            trigger_error("Can't save " . $this->getDstFolder() . 'Edit' . $this->getModelName() . '.xml"' . \PHP_EOL);
            return $status;
        }
